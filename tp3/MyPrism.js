	...
	initBuffers() {
		...
		while (stack_height < 1) {
			for (let i = 0; i < this.slices; i++) {
				...
				this.vertices.push(Math.cos(step * (i+1)), Math.sin(step * (i+1)), stack_height);
				this.vertices.push(Math.cos(step * (i+1)), Math.sin(step * (i+1)), stack_height+1/this.stacks);

				this.normals.push(Math.cos(step*i + step / 2), Math.sin(step*i + step/2), 0);
				this.normals.push(Math.cos(step*i + step / 2), Math.sin(step*i + step/2), 0);
				this.normals.push(Math.cos(step*i + step / 2), Math.sin(step*i + step/2), 0);
				this.normals.push(Math.cos(step*i + step / 2), Math.sin(step*i + step/2), 0);
			}
